    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                $deleteBtn = "<a class='btn btn-danger delete-item ml-2' href='" . route('admin.flash-sale.destroy', $query->id) . "'><i class='far fa-trash-alt'></i></a>";
                return $deleteBtn;
            })
            ...
            ->addColumn('show_at_home', function ($query) {
                if ($query->show_at_home == 1) {
                    $button = "<label class='custom-switch mt-2'>
                        <input type='checkbox' data-id='" . $query->id . "' checked name='custom-switch-checkbox' class='custom-switch-input change-at-home-status'>
                        <span class='custom-switch-indicator'></span>
                      </label>";
                } else {
                    $button = "<label class='custom-switch mt-2'>
                        <input type='checkbox' data-id='" . $query->id . "' name='custom-switch-checkbox' class='custom-switch-input change-at-home-status'>
                        <span class='custom-switch-indicator'></span>
                      </label>";
                }
                return $button;
            })
            ->rawColumns(['action', 'status', 'product_name', 'show_at_home'])
            ->setRowId('id');
    }
