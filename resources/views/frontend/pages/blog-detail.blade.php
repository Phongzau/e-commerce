                                            ...
                                            <div class="wsus__single_blog wsus__single_blog_2">
                                                ...
                                                <div class="wsus__blog_text">
                                                    <a class="blog_top red"
                                                        href="{{ route('blog-details', $blogItem->slug) }}">{{ $blogItem->category->name }}</a>
                                                    <div class="wsus__blog_text_center">
                                                        <a
                                                            href="{{ route('blog-details', $blogItem->slug) }}">{{ limitText($blogItem->title, 45) }}</a>
                                                        <p class="date">
                                                            {{ date('d M Y', strtotime($blogItem->created_at)) }}
                                                        </p>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            @foreach ($comments as $comment)
                                <div class="wsus__main_comment">
                                    <div class="wsus__comment_img">
                                        <img style="width: 80px; height: 88px; object-fit: contain;"
                        ...
                        <div class="wsus__post_comment">
                            ...
                            @if (auth()->check())
                                <form action="{{ route('user.blog.comment') }}" method="POST">
                                    ...
                                    <button class="common_btn">post comment</button>
                                </form>
                            @else
                                <p class="mb-3">Please login to comment on post!</p>
                            ...
                            @foreach ($moreBlogs as $item)
                                <div class="wsus__blog_post_single">
                                    ...
                                    <div class="wsus__blog_post_text">
                                        <a
                                            href="{{ route('blog-details', $item->slug) }}">{{ limitText($item->title, 45) }}</a>
                                        <p> <span>{{ date('M d Y', strtotime($item->created_at)) }} </span>
                                            {{ count($item->comments) }} Comment </p>
                                    </div>
                                </div>
                            @endforeach
